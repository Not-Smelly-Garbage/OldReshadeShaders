...
float3 paint_filter(in VSOUT i, in float pass_id)
{
	...
	for(int j = 0; j < NUM_DIRS; j++)
	{
		...
		float3 col_prev = tex2Dlod(ReShade::BackBuffer, float4(i.uv.xy - dir * NUM_STEPS_PER_PASS * ReShade::PixelSize, 0, 0)).rgb;

		for(int k = -NUM_STEPS_PER_PASS + 1; k <= NUM_STEPS_PER_PASS; k++)
		{
			float3 col_curr = tex2Dlod(ReShade::BackBuffer, float4(i.uv.xy + dir * k * ReShade::PixelSize, 0, 0)).rgb;
